# -*- coding: utf-8 -*-
# Download from https://github.com/itead/Sonoff_Devices_DIY_Tools
"""
    ...
    if listener.all_sub_num>0:
        ...
        for x in info_dict.keys():
            ...
            if info!= None:
                ...
                data_utf8 = {}
                for k in data:
                    k_utf8 = k.decode("UTF-8")
                    if k_utf8 == 'data1':
                        data_utf8 = ast.literal_eval(data[k].decode("UTF-8"))
                print('[ADDED] ' + x[8:18]+"  "+parseAddress(info.addresses[0]))
                device_info[x[8:18]] = { 'id': x[8:18], 'ip': parseAddress(info.addresses[0]), 'port': str(info.port),
                        'power': data_utf8['switch'], 'startup': data_utf8['startup'] }
    if len(listener.all_del_sub)>0:
            for x in listener.all_del_sub:
                cur_str=x[8:18]+"\nDEL"
                print('[NOTADDED]' + cur_str)
    return device_info
